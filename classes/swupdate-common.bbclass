...
def swupdate_add_artifacts(d, list_for_cpio):
    ...
    for image in images:
        ...
        if fstypes:
            ...
            noappend_swu_version = d.getVarFlag("SWUPDATE_IMAGES_NOAPPEND_SWU_VERSION", image, True)
            if noappend_swu_version == "0":  # Search for a file explicitly with SWU_VERSION
                imagebases = [ image + '-' + d.getVar('SWU_VERSION', True) ]
            elif noappend_swu_version == "1":  # Search for a file explicitly without SWU_VERSION
                imagebases = [ image ]
            else:  # None, means auto mode. Just try to find an image file with SWU_VERSION or without MACHINE
                imagebases = [ image + '-' + d.getVar('SWU_VERSION', True), image ]
...
def swupdate_create_cpio(d, swudeploydir, list_for_cpio):
    ...
    updateimage_link =  d.getVar('IMAGE_LINK_NAME', True) + '.lnk.swu'
